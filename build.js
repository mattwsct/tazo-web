      ...
      fetch(linksPath)
        ...
        .then(links => {
          links.filter(link => link.showOnHomepage).forEach(link => {
            ...
            const icon = link.icon ? '<img src="https://cdn.simpleicons.org/' + link.icon + '/fff" class="w-5 h-5" loading="lazy" alt="' + link.icon + ' logo" />' : '';
            const label = '<span' + (link.icon ? ' id="' + link.icon + '-lbl"' : '') + '>' + link.title + '</span>';
