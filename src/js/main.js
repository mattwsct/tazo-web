document.addEventListener('DOMContentLoaded', () => {
  ...
  const mountEmbed = (src,plat) => {
    ...
    glow.className=`stream-glow absolute -inset-2 blur-2xl opacity-20 ${
      plat==='kick'?'bg-green-500':'bg-purple-500'} animate-pulse rounded-xl z-0`;
    SW.querySelector('.relative.z-10').prepend(glow);

    SW.classList.remove('hidden');
    current=plat;
  };
  const clearEmbed = ()=>{SE.innerHTML='';SW.querySelector('.stream-glow')?.remove();SW.classList.add('hidden');current=null;};

  /* ── Debug panel ── */
  if (debugMode){
    const p=document.createElement('div');
    p.className='fixed top-4 right-4 bg-zinc-900 p-4 rounded-xl z-50 flex gap-3';
    p.innerHTML=`<button id="dbgKick" class="px-3 py-1 bg-green-600 text-white rounded">Show Kick</button>
    ...
    const upd=()=>{btnK.textContent=(kickLive?'Hide ':'Show ')+'Kick';
                   btnT.textContent=(twitchLive?'Hide ':'Show ')+'Twitch';};
    btnK.onclick=()=>{forceKick=!forceKick;refresh();upd();};
    btnT.onclick=()=>{forceTwitch=!forceTwitch;refresh();upd();};
  }
  ...
  async function refresh(){
    ...
    if(debugMode){kickLive=forceKick;twitchLive=forceTwitch;}
    else{
      const now=Date.now();
      if(now>kickFailUntil){
        try{kickLive=!!(await fetch(`https://kick.com/api/v2/channels/${kickUser}`).then(r=>r.json())).livestream;}
        catch{kickFailUntil=now+300000;}
      }
      if(now>twitchFailUntil){
        try{twitchLive=!/is\s+offline/i.test(await fetch(`https://decapi.me/twitch/uptime/${twitchUser}`).then(r=>r.text()));}
        catch{twitchFailUntil=now+300000;}
      }
    }

    setBadge('kick',kickLive); setBadge('twitch',twitchLive);
  ...
  fetch('links.json').then(r=>r.json()).then(data=>{
    data.filter(l=>l.showOnHomepage).forEach(l=>{
      const a=document.createElement('a');
      a.href=l.url; a.target='_blank';
      a.rel=['twitter','x','instagram','youtube','tiktok'].includes(l.id)?'me noopener':'noopener';
      a.className=`flex items-center gap-2 justify-center py-3 px-5 rounded-xl font-semibold bg-gradient-to-r ${l.bg}
      ...
      (l.liveOnly?a.classList.add('live-only-link','hidden'):0);
      (l.liveOnly? document.getElementById('liveLinks'):linksEl).appendChild(a);
    });
